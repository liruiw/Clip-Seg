...
class Fusion(nn.Module):
    ...
    def __init__(self, input_dim=3, proj=None):
        super().__init__()
        self.input_dim = input_dim
        self.x2_proj = nn.Linear(proj[0], proj[1]) if proj else None

    def tile_x2(self, x1, x2):
        if self.x2_proj:
            x2 = self.x2_proj(x2)

        x2 = x2[..., None, None].expand(-1, -1, x1.shape[-2], x1.shape[-1])
        return x2
...
class FusionAdd(Fusion):
    ...
    def __init__(self, input_dim=3, proj=None):
        super(FusionAdd, self).__init__(input_dim=input_dim, proj=proj)
...
class FusionMult(Fusion):
    ...
    def __init__(self, input_dim=3, proj=None):
        super(FusionMult, self).__init__(input_dim=input_dim, proj=proj)
...
class FusionMax(Fusion):
    ...
    def __init__(self, input_dim=3, proj=None):
        super(FusionMax, self).__init__(input_dim=input_dim, proj=proj)
...
class FusionConv(Fusion):
    ...
    def __init__(self, input_dim=3, proj=None):
        super(FusionConv, self).__init__(input_dim=input_dim, proj=proj)
...
class FusionConvLat(Fusion):
    ...
    def __init__(self, input_dim=3, output_dim=3, proj=None):
        super(FusionConvLat, self).__init__(input_dim=input_dim, proj=proj)
        self.conv = nn.Sequential(
            nn.ReLU(True),
...
class FusionFiLM(Fusion):
    ...
    def __init__(self, input_dim=3, output_dim=3, proj=None):
        super(FusionFiLM, self).__init__(input_dim=input_dim, proj=proj)
...
class FusionDeepConv(Fusion):
    ...
    def __init__(self, input_dim=3, proj=None):
        super(FusionDeepConv, self).__init__(input_dim=input_dim, proj=proj)
        self.conv = nn.Sequential(
            nn.ReLU(True),
            nn.Conv2d(input_dim * 2, input_dim, kernel_size=1, bias=False),
            nn.ReLU(True),
...
class FusionMultWord(nn.Module):
    ...
    def forward(self, x1, x2, x2_mask=None):
        ...
        for t in range(x2_len):
            x2_t = self.x2_proj(x2[:,t]) if self.x2_proj else x2[:,t]
...
class FusionSentenceAttention(nn.Module):
    ...
    def forward(self, x1, x2, x2_mask=None):
        ...
        x2_t = self.x2_proj(x2) if self.x2_proj else x2
        x2_t = x2_t.repeat(B, 1)

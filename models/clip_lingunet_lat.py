...
from models.resnet import IdentityBlock, ConvBlock
from models.unet import Up
from models.clip import build_model, load_clip

from models.fusion import FusionConvLat, FusionMult
...
class CLIPLingUNetLat(nn.Module):
    ...
    def __init__(self, output_dim, batchnorm):
        ...
        self._build_decoder()
        self.params_exclude_clip = list(self.parameters())
        self._load_clip()


    def _load_clip(self):
        self.clip_rn50 = build_model(load_clip("RN50", device="cpu")[0].state_dict())

    def _build_decoder(self):
        # language
        self.proj_input_dim = 1024
        self.lang_fuser1 = FusionMult(input_dim=self.input_dim // 2, proj=(self.proj_input_dim, 1024))
        self.lang_fuser2 = FusionMult(input_dim=self.input_dim // 4, proj=(self.proj_input_dim, 512))
        self.lang_fuser3 = FusionMult(input_dim=self.input_dim // 8, proj=(self.proj_input_dim, 256))
    ...
    def encode_image(self, img):
        ...
        return img_encoding.float(), [im.float() for im in img_im]
    ...
    def encode_text(self, tokens):
        ...
        return text_feat.float(), text_emb.float(), text_mask

    def forward(self, x, lat, tokens):
        in_shape = x.shape
        x, im = self.encode_image(x)

        l_enc, l_emb, l_mask = self.encode_text(tokens)
        l_input = l_enc
        ...
        x = self.lang_fuser1(x, l_input, x2_mask=l_mask)
        x = self.up1(x, im[-2])
        x = self.lat_fusion1(x, lat[-6])

        x = self.lang_fuser2(x, l_input, x2_mask=l_mask)
        x = self.up2(x, im[-3])
        x = self.lat_fusion2(x, lat[-5])

        x = self.lang_fuser3(x, l_input, x2_mask=l_mask)
        x = self.up3(x, im[-4])
...
if __name__ == '__main__':
    net = CLIPLingUNetLat(1, False).cuda()
    o = net(torch.zeros(1, 3, 256, 256).cuda())
